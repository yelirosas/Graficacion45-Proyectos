...
public class CamCap extends javax.swing.JFrame {
    ...
    class DaemonThread implements Runnable {
        ...
        public void run() {
            ...
            cjDetector = new CascadeClassifier(base_path + cjFile); // Se crea un objeto CascadeClassifier que reconocera caras
            cjDetections = new MatOfRect(); // Se inicializa el objeto donde se guardaran las caras detectadas
                ...
                while (runnable) {
                    if (webSource.grab()) {
                        try {
                            ...
                            cjDetector.detectMultiScale(frame, cjDetections); // Se buscan las caras dentro de la imagen y se guardan en faceDetections
                            
                            int i= 0;
                            
                            for (Rect rect : cjDetections.toArray()) {
                                ...
                                cj = frame.submat(cjDetections.toArray()[i]);
                                
                                
                                cjDetector.detectMultiScale(cj, cjDetections);

                                if (cjDetections.toArray().length != 0) {

                                    System.out.println("Caja café " + cjDetections.toArray().length);

                                    for (int j = 0; j < cjDetections.toArray().length; j++) {
                                        
                                        Rect rect2 = cjDetections.toArray()[j];

                                        Imgproc.rectangle(frame, new Point(rect2.x, rect2.y), new Point(rect2.x + rect2.width, rect2.y + rect2.height),
                                        new Scalar(0, 255, 0));
                                        
                                        Mat eye = cara.submat(cjDetections.toArray()[j]);
